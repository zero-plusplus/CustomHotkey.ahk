...
* `targetRect` : 検索の対象とする長方形の範囲。以下の3種類の指定方法があります。既定値は`"window"`です
  * 以下の文字列を指定する
    * `"screen"` : アクティブなスクリーンを対象にします
    * `"window"` : アクティブなウィンドウを対象にします
  * 指定サイズの四角形を検索する
    * `x` : 検索範囲の始点となるx座標
    * `y` : 検索範囲の始点となるy座標
    * `width` : 四角形の横幅
    * `height` : 四角形の縦幅
    * `origin` : `x`及び`y`座標の原点。指定方法については[座標の原点](#座標の原点)を見てください。既定値は`"window"`です
  * 指定した位置(左上)から位置(右下)までの長方形の範囲を検索する。以下のフィールドを持つオブジェクトの長さが2の配列を指定します
    * `x` : 検索範囲の始点となるx座標
    * `y` : 検索範囲の始点となるy座標
    * `origin` : `x`及び`y`座標の原点。指定方法については[座標の原点](#座標の原点)を見てください。既定値は`"window"`です
* `button` : クリックに使うマウスのボタン名。以下の文字列を指定できる。大文字小文字は区別しません。既定値は`"LButton"`です
...
* `"monitor"`, `"screen"` : アクティブモニターの左上を原点に設定します
...
* 以下の専用の変数を利用できます
  ...
  * `{Monitor}` : プライマリモニタの境界座標を`"top,left,right,bottom"`の形式で埋め込みます。以下の特徴があります
    ...
    * 末尾にモニタ番号を表す数値を指定することで特定のモニターを対象にできます。省略した場合はプライマリモニタが対象になります
