...
* `mousepos` : [矩形](#矩形)または`"window-top-left-corner"`のように以下の接頭辞および接尾辞を組み合わせた文字列を指定します
  1. 以下の接頭辞を指定します。省略された場合は`"monitor"`を指定した場合と同じです
    ...
    * `"monitors"`, `"screens"` : 仮想モニタを対象とします。この仮想モニタは接続しているすべてのモニタを1つとして扱います
...
`monitors`(別名`screens`)は接続しているすべてのモニタを仮想的に1つとして扱います。以下は3つのモニタを矩形にならないように配置した場合の図解です。
...
* `"monitor"`, `"screen"` : アクティブモニタの左上を原点に設定します。`"monitor-primary"`のように以下の接尾辞を指定することで対象のモニタを変更できます
  * `-<number>` : 指定した番号のモニタの左上を原点に設定します。例: `"monitor-1"`
  * `-primary` : プライマリモニタの左上を原点に設定します。例: `"monitor-primary"`
* `"monitors"`, `"screens"` : 接続しているすべてのモニタを1つとして扱う仮想モニタの左上を原点に設定します。絶対座標を指定したい場合は`"monitor"`ではなくこちらを指定することをおすすめします
...
`"monitors"`(別名`"screens"`)は接続されているすべてのモニタを仮想的に1つのモニタとして扱います。以下は3つのモニタを矩形でない形に配置している場合の接尾辞による調整の図解です。
...
* 以下のフィールドを持つオブジェクト。例: `{ x: 0, y: 0, width: 30, height: 30 }`
  ...
  * `width` : 矩形の横幅を表す数値
  * `height` : 矩形の縦幅を表す数値
  * `origin` : `x`または`y`[座標の原点](#座標の原点)。省略時は`"monitors"`です
...
* 以下の2または3つの要素を持つ配列の配列。例: `[ [ 0, 0 ], [ 30, 30 ] ]`
  1. 始点または終点のx座標を表す数値
  2. 始点または終点のy座標を表す数値
  3. [座標の原点](#座標の原点)。省略時は`"monitors"`です
